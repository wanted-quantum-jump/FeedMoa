...
@Component
public class DateStatisticsCalculator implements StatisticsCalculator {
    ...
    public StatisticsResponse calculate(List<Statistics> statisticsList, String value) {
        ...
        for (Statistics stat : statisticsList) {
            String dateKey = format.format(stat.getPostTime());
            statisticsMap.putIfAbsent(dateKey, 0L);
            updateStatisticsMap(statisticsMap, stat, value, dateKey);
        }
